...
def audit_sheet(frames: list[Image.Image], output: Path) -> None:
    size = 256
...
def main() -> None:
    ...
    parser.add_argument("sheets", nargs=6, type=Path)
    ...
    for sheet_path in args.sheets:
        ...
        for column in range(8):
            left = round(column * sheet.width / 8)
            right = round((column + 1) * sheet.width / 8)
            cell = sheet.crop((left, 0, right, sheet.height))
            cell.save(raw_dir / f"frame-{index:03d}.png")
            frame = normalize(cell)
            frame.save(frame_dir / f"frame-{index:03d}.png")
            frames.append(frame)
            index += 1

    audit_sheet(frames, args.output_dir / "niulai-walk-48-hand-authored-audit-v7.png")
    print(frame_dir)
